...
def sorted_merge_posts_and_tfs(posts_tfs1, posts_tfs2):
    ...
    while idx_tfs1 < len(posts_tfs1) and idx_tfs2 < len(posts_tfs2):
        doc_id_tfs1 = posts_tfs1[idx_tfs1][0]
        doc_id_tfs2 = posts_tfs2[idx_tfs2][0]
        tf_tfs1 = posts_tfs1[idx_tfs1][1] 
        tf_tfs2 =  posts_tfs2[idx_tfs2][1]
        if doc_id_tfs1 == doc_id_tfs2:
            lst_result.append((doc_id_tfs1, tf_tfs1+tf_tfs2))
            idx_tfs1+=1
            idx_tfs2+=1
        elif doc_id_tfs1 < doc_id_tfs2:
            lst_result.append((doc_id_tfs1,tf_tfs1))
            idx_tfs1+=1
        else:
            lst_result.append((doc_id_tfs2, tf_tfs2))
            idx_tfs2+=1
    
    while idx_tfs1 < len(posts_tfs1):
        doc_id_tfs1 = posts_tfs1[idx_tfs1][0]
        tf_tfs1 = posts_tfs1[idx_tfs1][1] 
        lst_result.append((doc_id_tfs1, tf_tfs1))
        idx_tfs1+=1
    while idx_tfs2 < len(posts_tfs2):
        doc_id_tfs2 = posts_tfs2[idx_tfs2][0]
        tf_tfs2 = posts_tfs2[idx_tfs2][1] 
        lst_result.append((doc_id_tfs2, tf_tfs2))    
        idx_tfs2+=1   
