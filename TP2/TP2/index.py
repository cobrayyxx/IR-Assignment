...
class InvertedIndex:
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        for i in self.doc_length.values():
            sum_result+=i
        self.average_doc_length = sum_result/len(self.doc_length)
...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        length_byte_postings = self.postings_dict[term][2]
        length_byte_tf = self.postings_dict[term][3]

        self.index_file.seek(start_index,0)

        result_postings_list = self.index_file.read(length_byte_postings)
        result_postings_list = self.postings_encoding.decode(result_postings_list)

        result_tf = self.index_file.read(length_byte_tf)
        result_tf = self.postings_encoding.decode_tf(result_tf)

        return (result_postings_list, result_tf)
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        number_of_postings_list = len(postings_list)

        self.postings_dict[term] = (start_position, number_of_postings_list, 
                                    len(encoded_postings_list), len(encoded_tf_list))
        

        for doc_id in enumerate(postings_list):
            try:
                self.doc_length[doc_id[1]] += tf_list[doc_id[0]] 
            except:
                self.doc_length[doc_id[1]] = tf_list[doc_id[0]]
