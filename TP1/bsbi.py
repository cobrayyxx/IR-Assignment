...
class BSBIIndex:
    ...
    def parse_block(self, block_dir_relative):
        ...
        for file in os.listdir(file_dir):
            doc_dir = os.path.join(file_dir, file)
            doc_id = self.doc_id_map.__getitem__(doc_dir)
            with open(doc_dir, 'r' ) as f:
                sentence = f.read()
                new_sentence = re.sub(r'[^\w\s]', ' ', sentence).lower()
                clear_stop = stopword.remove(new_sentence)
                tokens = stemmer.stem(clear_stop)
                tokens = tokens.split()
                for token in tokens:
                    doc_id = self.doc_id_map.__getitem__(doc_dir)
    ...
    def merge(self, indices, merged_index):
        ...
        for current_term,current_posting in heapq.merge(*indices):
            if current_term > prev_term:
                lst = list(set(posting_list))
                posting_list = lst
                posting_list.sort()
                merged_index.append(prev_term, posting_list)
                posting_list = current_posting
                prev_term = current_term
            elif current_term == prev_term:
                posting_list += current_posting
    ...
    def retrieve(self, query):
        ...
        clear_stop = stopword.remove(query)
        tokens = stemmer.stem(clear_stop)

        lst_query = tokens.split()
        result = []

       
        self.load()

        with InvertedIndexReader(self.index_name, self.postings_encoding, self.output_dir) as idx_reader:
            for q in lst_query:
                posting_list_q = idx_reader.get_postings_list(self.term_id_map[q])

                if len(result) != 0 and posting_list_q is not None:
                    result = sorted_intersect(result, posting_list_q)
                elif len(result) == 0 and posting_list_q is not None:
                    result = posting_list_q
                       
        list_file = [self.doc_id_map[res] for res in result]
        return list_file
