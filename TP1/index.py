    ...
    postings_dict: Dictionary mapping:
        ...
        yang memetakan term ID (integer) ke 3-tuple:
           1. start_position_in_index_file : (dalam satu bytes) posisi dimana
              ...
              menggunakan operasi "seek" untuk mencapainya. (seek itu menuju ke posisis tertentu)
...
class InvertedIndexReader(InvertedIndex):
    ...
    """
    def __iter__(self):
        self.count=0 #### used for couting in __next__ while iterating #####
        return self
    ...
    def __next__(self):
        ...
        """
        # TODO
        self.limit = len(self.postings_dict)
        if self.count < self.limit:
            term_id = self.terms[self.count]
            posting_list = self.get_postings_list(term_id)
            self.count+=1
    ...
    def get_postings_list(self, term):
        ...
        start_index = self.postings_dict[term][0]
        length_byte = self.postings_dict[term][2]
        self.index_file.seek(start_index,0) # param 0 --> dari awal file
        result_posting_list = self.index_file.read(length_byte)
        result_posting_list = self.postings_encoding.decode(result_posting_list)

        
        return result_posting_list
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list):
        ...
        2. Menyimpan metadata dalam bentuk self.terms dan self.postings_dict. {||| bisa pake compresison}
        ...
        start_position = self.index_file.seek(0,2) #.tell buat ngasih tau pointer sekarang
            
        number_of_postings_in_list = len(postings_list)

        self.postings_dict[term] = (start_position, number_of_postings_in_list, len(encoded_list))

        self.index_file.seek(0,2)   #.seek buat ngarahin pointer di file .index
...
if __name__ == "__main__":
    ...
    with InvertedIndexWriter('test', postings_encoding=StandardPostings, directory='./tmp/') as index:
        ...
        assert StandardPostings.decode(index.index_file.read(index.postings_dict[1][2])) == [2, 3, 4, 8, 10], "posisi postings salah"
        assert StandardPostings.decode(index.index_file.read(index.postings_dict[2][2])) == [3, 4, 5], "posisi postings salah"
    ...
    with InvertedIndexWriter('test', postings_encoding=VBEPostings, directory='./tmp/') as index:
        ...
        assert VBEPostings.decode(index.index_file.read()) == [2, 3, 4, 8, 10, 13, 14, 15], "penyimpanan postings pada harddisk salah"
    ...
    with InvertedIndexReader('test', postings_encoding=VBEPostings, directory='./tmp/') as index:
        # print(index.get_postings_list())
        index.index_file.seek(0)
        for i in index:
            print(i)